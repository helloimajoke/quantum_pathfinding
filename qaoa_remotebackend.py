...
if not IBM_QUANTUM_TOKEN:
    raise ValueError("IBM Quantum token not found in .env file")

from qiskit.providers.ibmq import IBMQ
IBMQ.save_account(IBM_QUANTUM_TOKEN)

# ====================== CONFIGURATION ======================
# Custom weights for edges (modify these values)
# ====================== LARGER GRAPH CONFIGURATION ======================
# Custom weights for edges (modify these values)
CUSTOM_WEIGHTS = {
    (0, 1): 2.4,
    (0, 2): 2.8,
...
def run_qaoa(G, source, target, use_ibm=False):
    ...
    if use_ibm:
        try:
            ...
            IBMQ.save_account(os.getenv("IBM_QUANTUM_TOKEN"))
            IBMQ.load_account()
            
            # Get IBM Kyoto backend
            provider = IBMQ.get_provider(hub='ibm-q')
            backend = provider.get_backend('ibm_kyoto')
            print(f"Using IBM Quantum backend: {backend.name}")
        except Exception as e:
            print(f"IBM Quantum access failed: {str(e)}")
    ...
    quantum_instance = QuantumInstance(
        backend,
        shots=1024,  # Increased shots for real hardware
        optimization_level=3  # Aggressive circuit optimization
    )

    # Configure QAOA
    qaoa = QAOA(
        optimizer=COBYLA(maxiter=50),
        reps=1,  # Fewer layers for shorter circuits
        quantum_instance=quantum_instance,
        initial_point=[0.1, 0.1]  # Better parameter initialization
    )
...
if __name__ == "__main__":
    ...
    # Run QAOA on IBM Kyoto
    path_edges, cost, qaoa_time = run_qaoa(G, 0, 3, use_ibm=True)
    print(f"QAOA on IBM Kyoto - Cost: {cost}, Time: {qaoa_time}s")
